...
type QuizResultProps = {
  ...
  /** `onboarding` : même détail des catégories que le bilan, avec CTA fixé en bas (fin du quiz d’accueil). */
...
type CategoryBreakdownSectionProps = {
  ...
  comparisonByCategoryId: Map<string, QuizOnboardingBilanCategory>
  categoryBarScaleMaxT: number | undefined
}
...
export function QuizResult({
  ...
  if (isOnboarding) {
    return (
      <div className="flex h-full min-h-0 w-full flex-col bg-[#f0f7f0]">
        <div className="mx-auto min-h-0 w-full max-w-lg flex-1 overflow-y-auto overscroll-y-contain pb-[calc(7.5rem+env(safe-area-inset-bottom,0px))]">
          <header className="px-4 pt-4 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both motion-reduce:animate-none motion-reduce:opacity-100 motion-reduce:translate-y-0">
          ...
          <div
            className={cn(
              'relative w-full px-4 pt-2',
              'animate-in zoom-in-95 fade-in duration-700 ease-out fill-mode-both [animation-delay:90ms]',
          ...
          {categories && categories.length > 0 ? (
            <CategoryBreakdownSection
              ...
              comparisonByCategoryId={comparisonByCategoryId}
              categoryBarScaleMaxT={categoryBarScaleMaxT}
            />
          ) : null}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-[#1A4D3E]/10 bg-[#f0f7f0] px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4">
          <Button
            type="button"
