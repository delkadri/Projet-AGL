...
export class ScoreHistoryCategoryDto {
    @ApiProperty({ description: 'Identifiant de la catégorie', example: 'transport' })
    id!: string;

    @ApiProperty({ description: 'Libellé de la catégorie', example: 'Transport' })
    name!: string;

    @ApiProperty({ description: 'Total kgCO2e/an pour cette catégorie', example: 2150.4 })
...
export class ScoreHistoryResponseDto {
    ...
    @ApiProperty({
        description: 'Totaux par catégorie au moment de l’enregistrement (peut être null pour les anciennes entrées).',
