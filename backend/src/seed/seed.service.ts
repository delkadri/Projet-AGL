...
export class SeedService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log(
      'Démarrage de la synchronisation des données de base (Parcours, Niveaux)...',
    );
    try {
      ...
      this.logger.error(
        `Erreur lors de la synchronisation des données: ${error.message}`,
      );
    }
  }
    ...
    const parcours = [
      {
        ...
        name: 'DÉCOUVERTE',
        description:
          'L’essentiel rapidement. Comprendre mon score sans surcharge avec des défis simples.',
        defis_per_period: 1,
        quizz_per_period: 1,
        period_type: 'WEEK',
      },
